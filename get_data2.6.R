...
getSymbols("^DJI", from = as.Date("1987-01-01"),to = "2022-03-22", src = "yahoo")
...
#CHOSING INPUTS; do N-1 (window size - 1) in the input for N -----> NO (FOR now)
measure_data <- function(index_name, closing_column, starting_date, N, interval_T){
  ...
  SP_increments <- index_sub_dataset %>% 
    ...
    compute("SP_increments")
                                                                          #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  total_increments <- SP_increments %>% select(abs_increments) %>% sum(na.rm = TRUE)   #add na.rm here #ADDED ADDDDDDDDDDDDDDDD THISSSSSSSSSSSSSSSSS
...
measure_data(DJI, 4, "2007-08-09", 20, 1)
...
partition_function_data <- function(index_name, closing_column, starting_date, N, interval_T){
  ...
  for(q in seq(from=-5.033, to=5.033, by=0.033)){
    ...
    powers = sum('^'(SP_increments$measure,q), na.rm = TRUE)  #NA.RM = TRUE hence missing values are counted as 0 in the sum 
    output[i,1] <- i
    output[i,2] <- q
    output[i,3] <- powers
    i = i + 1
...
dynamic_eta <- function(total_shifts, index_name, closing_column, starting_date, N, interval_T){
  ...
  for(i in seq(from = 1, to = total_shifts + 1, by = 1)){      #did until totalshifts+1, before it was just total_shifts
...
AVR_with_time <- function(total_shifts, index_name, closing_column, starting_date, N, interval_T){
  ...
  eta <- dynamic_eta(total_shifts, index_name, closing_column, starting_date, N, interval_T) %>% 
    mutate(end_day = shift + (N+1)) #CHANGED from N to N+1
...
#the number of data points to look back for a specific crash in terms of shifts and windows;
#for 14 points before the crash, with window of 4, perform 10 shifts for the last shift to encompas the crash 
...
raw_data(DJI, 4, "2022-03-02", 4, 11)

lookback_date(DJI, 4, "2022-03-21", 14)

#total_shifts, index_name, closing_column, starting_date, N, interval_T
AVR_with_time(20, DJI, 4, "2022-03-02", 4, 1)

AVR_with_time(1, DJI, 4, "2022-03-07", 4, 1)

dynamic_eta(1, DJI, 4, "2022-03-02", 4, 1)

measure_data(DJI, 4, "2022-03-16", 4, 1)

auc_cq_function(1, DJI, 4, "2022-03-16", 4, 1)
dynamic_eta(1, DJI, 4, "2022-03-02", 4, 1)
specific_heat_function(DJI, 4, "2022-03-02", 4, 1)

ggplot(specific_heat_function(DJI, 4, "2022-03-16", 4, 1)) +
  geom_line(aes(x = q, y= Cq))

#testing whole code on real time now; 
measure_data(DJI, 4, "2022-03-16", 4, 1)

partition_function_data(DJI, 4, "2022-03-16", 4, 1)

specific_heat_function(DJI, 4, "2022-03-16", 4, 1)

AUC(DJI, 4, "2022-03-16", 4, 1)

auc_cq_function(15, DJI, 4, "2022-03-02", 4, 1)

dynamic_eta(2, DJI, 4, "2022-03-15", 4, 1)

AVR_with_time(2, DJI, 4, "2022-03-15", 4, 1)

AVR_with_time(15, DJI, 4, "2022-03-02", 4, 1)



lookback_date(DJI, 4, "2022-03-21", 20)
AVR_with_time(20, DJI, 4, "2022-02-22", 4, 1)

0.0049814156
#------TESTS--------------------------------------------------------------------

  
  auc_cq_table <- auc_cq_function(10, DJI, 4, "2022-03-02", 4, 1)
  #-----------------------------------------------------------------
  av_table <- auc_cq_table %>% mutate(average_An = NA)
...
av_table[1:2, 4]
#-------------------------------------------------------------------------------
#DYNAMIC ETA LOOKS FINE; CHECK THE AUC_CQ FUNCTION!!!
...
#checking partriton function !!!!!!!!!!!!---i think its fine 
partition_function_data <- function(index_name, closing_column, starting_date, N, interval_T){
  ...
  for(q in seq(from=-5.033, to=5.033, by=0.033)){
    ...
    powers = sum('^'(SP_increments$measure,q), na.rm = TRUE)  #need to change SP_increments to the table outputted by measure_data
    output[i,1] <- i
    output[i,2] <- q
    output[i,3] <- powers
    i = i + 1
...
#TESTING the above -------------------------it works woohoo


auc_cq_table <- auc_cq_function(1, DJI, 4, "2022-03-16", 4, 1)
auc_cq_table
av_table <- auc_cq_table %>% mutate(average_An = NA)
av_table

for(i in seq(from = 1, to = 1 +1, by = 1)){      #did until totalshifts+1, before it was just total_shifts
...
#--------------------------------------------
dynamic_eta(1, DJI, 4, "2022-03-16", 4, 1)
