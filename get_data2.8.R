...
dynamic_eta <- function(total_shifts, index_name, closing_column, starting_date, N, interval_T){
  ...
  for(i in seq(from = 1, to = total_shifts + 1, by = 1)){  #ADDED A PLUS 1 HERE-double check again please 
...
AVR_with_time <- function(total_shifts, index_name, closing_column, starting_date, N, interval_T){
  ...
  timed_AVR <- eta %>% 
    ...
    filter(!is.na(end_date))%>%   #filter which cuts off the end dates which are not available in the time seies
    #this is needed for when running the AVR in real time
    compute("timed_AVR")
...
ggplot(dynamic_eta(200, DJI, 4, as.Date("2003-12-23"), 1000, 1), aes(x = n, y= eta)) +
...
AVR_with_time(16, DJI, 4, as.Date("2022-03-02"), 4, 1)
ggplot(AVR_with_time(16, DJI, 4, as.Date("2022-03-02"), 4, 1)) +
  geom_point(aes(x = n, y= eta))
...
DJI_df %>% head()
partition_function_data(DJI, 4, "2001-12-22", 1000, 1) %>% View()
View(measure_data(DJI, 4, "2001-12-22", 1000, 1))

#AVR for extended time period; running AVR on the whole DJI dataset, from 1992-01-02 until today; 23rd march 2022 (6308 data points)
AVR_2000 <- dynamic_eta(200, DJI, 4, as.Date("2000-12-22"), 1000, 1)
ggplot(AVR_2000, aes(x = n, y= eta)) +
  geom_point() + geom_line()


AVR_DJI <- AVR_with_time(5000, DJI, 4, as.Date("1992-12-22"), 1000, 1)
View(AVR_DJI)
ggplot(AVR_DJI, aes(x = n, y= eta)) +
  geom_point() + geom_line()

DJI_df %>% head()

DJI_df %>% filter(date>="2000-12-22" & date <= "2001-12-22") %>% filter(is.na(DJI.Close))
#its not working- WHY;

View(measure_data(DJI, 4, "1992-12-22", 1000, 1))


#HERE IS THE PROBLEM; ZQN is inf----why????? check and change!!!--compare to measure at 2001-why has this happened?- today it was correct- what the heck 
View(partition_function_data(DJI, 4, "1992-12-22", 1000, 1))

# testing parttion function 

#------------------------------------------------------------CHECKS 
  
  SP_increments <- measure_data(DJI, 4, "2000-12-22", 1000, 1)
  ...
  sum('^'(SP_increments$measure, -5.033))

  SP_increments %>% select("measure") %>% mutate(measure2 = '^'(measure, -5.033))

  '^'(SP_increments$measure, -5.033)
