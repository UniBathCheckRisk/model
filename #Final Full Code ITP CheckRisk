...
getSymbols(c("^HSI", "^FTSE", "BTC-USD", "GC=F", "GLD"), from = "1970-01-01",
           to = "2022-07-01")
#!!!!!!!!!!!!!!USE na.omit() to get rid of missing values in ^HSI, ^FTSE, GC=F

# e.g. new HSI table- do this before inputting into functions
View(na.omit(HSI))
HSI <- HSI %>% na.omit() #do this before inputting HSI as an argument into functions 
...
#----------------------------------------------
#importing noise data from excels
...
noise_DJI_df <- noise_DJI_df %>%
  ...
  mutate(noise_DJI_price = round(noise_DJI_price, 2))                 #NEED to round data to 2 d.p- avoid truncation errors
noise_DJI_df %>% head()
...
# CHANGING input parameters- add white noise to the function & change the function where it is used below (3 times I believe) !!!!!!!!!!!!!!!!!!!
AVR_stat_test <- function(total_shifts, index_name, closing_column, starting_date, N, interval_T, AVR_windows, noise_data){
  
  next_date <- starting_date 
  
  # running AVR on noise 
  AVR_noise <- AVR_with_time(total_shifts, noise_data, closing_column, next_date, N, interval_T)
  
  #original AVR time series run;
  AVR_ts <- AVR_with_time(total_shifts, index_name, closing_column, next_date, N, interval_T) %>% 
    mutate(condition_AVR = ifelse(eta > 10^-3, 1, 0)) %>%                #our definition; AVR jumps over 10^-3 threshold
    mutate(AVR_window = 1)
  
  #joining AVR on ts and AVR on noise via end_date
  AVR_first <- AVR_ts %>%
    left_join(AVR_noise %>%
                select(end_date, "noise_eta" = "eta"),
              by = "end_date") %>%
    mutate(condition_AVR_noise = ifelse(noise_eta >= eta, 0, 1)) %>%     #if AVR on noise is greater than or equal to AVR on time series then AVR has not jumped (0)
    mutate(condition_AVR = ifelse(condition_AVR == 1 & condition_AVR_noise == 1, 1, 0))
...
AVR_stat_3 <- AVR_stat_test(20, DJI_whole, 1, "1992-01-02", 1000, 1, 4, DJI_noise)
View(AVR_stat_3)
...
#--------------------------------ADD THIS to the function with final stat test- comparing AVR and ts- no, do what I was doing before 
#ADDING noise data to the AVR_stat function for another AVR threshold

#first, running AVR on DJI_noise; 
DJI_noise 


AVR_noise_DJI <- AVR_with_time(20, DJI_noise, 1, "1992-01-02", 1000, 1)

View(AVR_noise_DJI)

#original AVR stat threshold run; 
AVR_no_noise <- AVR_with_time(20, DJI_whole, 1, "1992-01-02", 1000, 1) %>% 
  mutate(condition_AVR = ifelse(eta > 10^-3, 1, 0))%>%                #our definition; AVR jumps over 10^-3 threshold
  mutate(AVR_window = 1)
...
AVR_first <- AVR_no_noise %>%
  left_join(AVR_noise_DJI %>%
              select(end_date, "noise_eta" = "eta"),
            by = "end_date") %>%
  mutate(condition_AVR_noise = ifelse(noise_eta >= eta, 0, 1)) %>%     #if AVR on noise is greater than or equal to AVR on time series then AVR has not jumped (0)
  mutate(condition_AVR = ifelse(condition_AVR == 1 & condition_AVR_noise == 1, 1, 0))
...
#change AVR first and next to diff things in the function above (AVR stat) THEN avr First and next will be the joined df 
...
AVR_TS_stat_test_1 <- function(total_shifts, index_name, closing_column, starting_date, N, interval_T, AVR_windows, drop_period, drop, index_quotes, noise_data){
  
  AVR_stat_1 <- AVR_stat_test(total_shifts, index_name, closing_column, starting_date, N, interval_T, AVR_windows, noise_data)
...
DJI_stat_test <- AVR_TS_stat_test_1(20, DJI_whole, 1, "1992-01-02", 1000, 1, 4, 5, 10, "DJI_whole", DJI_noise)
DJI_stat_test

# <- AVR_TS_stat_test_1(20, GSPC, 4, "1992-01-02", 1000, 1, 4, 5, 10, "GSPC", SP_noise)  - not working in this version 
SP_stat_test <- AVR_TS_stat_test_1(20, SP_whole, 1, "1992-01-02", 1000, 1, 4, 5, 10, "SP_whole", SP_noise)
SP_stat_test

#number of trails is the max number in the AVR_windows column and sucesses are the sum of the 1's 
sucesses <- SP_stat_test %>% summarise(total_sucessessful_trails = sum(Success)) %>% data.frame()
trails <- SP_stat_test %>% summarise(number_of_trails = max(AVR_window)) %>% data.frame()
...
#takes longer time to run these; 
SP_stat_test_2 <- AVR_TS_stat_test_1(50, SP_whole, 1, "1992-01-02", 1000, 1, 20, 5, 10, "SP_whole", SP_noise)
SP_stat_test_2

#checking below--------------------------------------------Now it works-----
SP_stat_test100 <- AVR_TS_stat_test_1(100, SP_whole, 1, "1992-01-02", 1000, 1, 40, 5, 10, "SP_whole", SP_noise)
