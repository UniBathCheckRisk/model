...
partition_function_data <- function(index_name, closing_column, starting_date, N, interval_T){
  ...
  iterations = 427   #427 when from -7.033 to 7.033 and 306 when from -5.033 to 5.033
  variables = 3
  i <- 1
  output <- matrix(ncol=variables, nrow=iterations)
  
  # Creating empty data frame to populate the sums for the different values of q
  for(q in seq(from=-7.033, to=7.033, by=0.033)){
...
2006-06-14
  ...
  SP_increments <- measure_data(DJI, 4, "1992-01-02", 1000, 1)
  SP_increments
  ...
  SP_increments %>% mutate(measure2 = '^'(measure,-5.033)) %>%
    mutate(abs_increments = ifelse(abs_increments == 0, 0.005, abs_increments)) %>% filter(abs_increments == 0.005)
