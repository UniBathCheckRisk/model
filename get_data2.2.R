...
barChart(time_series_data(FTSE))
...
testing <- auc_cq_function(50, DJI, 4, "2007-01-03", 1000, 1) %>% head(20)
testing

ma3 <- rollapplyr(testing[,4],list(-(3:1)),mean,fill=NA)

ma3
testing[,4]

#-----------WRITING a for loop below;-------------------------------------
...
av_table <- testing %>% mutate(average_An = NA)

for(n in seq(from = 1, to = 20, by = 1)){
  
  average_an <- (sum(testing[1:n-1,4]))/(n-1) 
  
  av_table[n,5] <- average_an
  }

testing[9,4]  #4th column outputs A(n)
sum(testing[1:2, 4])

av_table %>% mutate(average_An = ifelse(average_An = )) 
class(av_table[1,5])
#---------------------MODULATING the above, below;---------------------

# function outputting a table which indcludes the 'eta' as seen on fondseca 
dynamic_eta <- function(total_shifts, index_name, closing_column, starting_date, N, interval_T){
  ...
  for(i in seq(from = 1, to = total_shifts, by = 1)){
...
dynamic_eta(50, DJI, 4, "2007-01-03", 1000, 1)
#-----------^ above is wrong, try again below; 

auc_cq_table <- auc_cq_function(50, DJI, 4, "2007-01-03", 1000, 1)
auc_cq_table
