...
getSymbols("^GSPC", from = as.Date("1929-01-01"),to = "1992-12-31", src = "yahoo")
...
historic_SP_DJI_df <- historic_SP_DJI_df %>%
  select("SP_dates" = "Dates...1", "SP_closing" = "PX_LAST.S.P.500",
...
historic_SP <- historic_SP_DJI_df %>% select(SP_dates, SP_closing)
...
#OUTPUTS---------------------------------------------------------------------------------------------
#specific heat replicating canessa on S&P dataset; removing black Monday; the 1970th point after 1980
...
SP_1980_1970 <- specific_heat_function(GSPC, 4, "1980-01-01", 1969, 1) #points up to excl crash

#CHECK THIS!!!!!!!!!!!!---lower green lobe when 1972 output and higher green lobe when 1971 output
...
SP1980_p1 <- ggplot(data = SP_1980_499, color = "black", aes(x = q, y= Cq)) +
  ...
  geom_line(data= SP_1980_1970, color = "blue") + 
  geom_point(data= SP_1980_1970, color = "blue") +
  geom_line(data= SP_1980_1969, color = "green") + 
  geom_point(data= SP_1980_1969, color = "green") +
  ggtitle("Specific Heat Curve for S&P500 1980-1992, black = 499 pts, red = 3287 pts, blue = 1970 pts,
